...
foreach ($line in $content) {
    ...
    if ($inTable -and $line -match '^\|') {
        ...
        if ($columns.Count -ge 3) {
            ...
            # Skip empty case names or if status/testStatus contains "invalid", "Skipped", or "test success"
            $shouldSkip = $false
            if ($status -match 'invalid' -or $testStatus -match 'invalid') {
                $shouldSkip = $true
            }
            if ($status -match 'Skipped') {
                $shouldSkip = $true
            }
...
if ($rerun) {
    # In rerun mode, only skip cases with "invalid" or "Skipped" status
    $casesToRun = $testCases | Where-Object { 
        $_.Status -notmatch 'invalid' -and 
        $_.TestStatus -notmatch 'invalid' -and 
        $_.Status -notmatch 'Skipped' 
    }
    Write-Host "Running in RERUN mode - will test all cases except 'invalid' and 'Skipped'" -ForegroundColor Yellow
} else {
    # In normal mode, skip cases with "invalid", "Skipped", or "test success"
    $casesToRun = $testCases | Where-Object { -not $_.ShouldSkip }
    Write-Host "Running in NORMAL mode - will skip 'invalid', 'Skipped', and 'test success' cases" -ForegroundColor Green
}
