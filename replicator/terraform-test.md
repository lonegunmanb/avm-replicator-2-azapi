...
**PowerShell**:
```powershell
Remove-Item .terraform, .terraform.lock.hcl, *.json, err.log, fix.log -Recurse -Force -ErrorAction SilentlyContinue
```

**Bash**:
```bash
rm -rf .terraform .terraform.lock.hcl *.json err.log fix.log
```
...
**Possible Results**:
- ✅ **Success**: Retrieve resource status, then proceed to verify idempotency
- ❌ **Failed**: Analyze error before marking invalid

**If apply succeeds, retrieve the deployed resource status**:

The agent must use the Azure CLI to retrieve the resource declared in `azurerm.tf` and save its status in JSON format:

1. **Identify the resource** from `azurerm.tf` (e.g., resource type and name)
2. **Generate the appropriate `az` command** using the MCP tool to retrieve the resource:
   - Use `mcp_azure_mcp_ser_extension_cli_generate` tool with intent describing what resource to retrieve
   - Example intent: "Get the details of the virtual network resource in JSON format"
3. **Execute the generated command** and save the output to `azurerm.json`

**Note**: The agent should use the MCP `extension_cli_generate` tool to generate the exact command based on the resource type in `azurerm.tf`.
...
1. **If plan shows NO changes**:
   ...
   - **Verify actual resource hasn't been modified**: Retrieve resource status and compare to `azurerm.json`
     
     Use the same Azure CLI command from Step 2 to retrieve the current resource status:
     
     **PowerShell**:
     ```powershell
     # Use the same az command from Step 2 (generated by MCP tool)
     az network vnet show --resource-group <rg_name> --name <vnet_name> --output json | Out-File -FilePath azapi_step3.json -Encoding utf8
     ```
     
     **Bash**:
     ```bash
     # Use the same az command from Step 2 (generated by MCP tool)
     az network vnet show --resource-group <rg_name> --name <vnet_name> --output json > azapi_step3.json
     ```
     ...
     **PowerShell**:
     ```powershell
     # Filter out insignificant fields recursively from both JSON files
     jq --sort-keys 'walk(if type == "object" then del(.etag, .provisioningState, .resourceGuid, .timeCreated, .changedTime) else . end)' azurerm.json > azurerm_filtered.json
     jq --sort-keys 'walk(if type == "object" then del(.etag, .provisioningState, .resourceGuid, .timeCreated, .changedTime) else . end)' azapi_step3.json > azapi_step3_filtered.json
     
     # Compare the filtered files
     $differences = Compare-Object (Get-Content azurerm_filtered.json) (Get-Content azapi_step3_filtered.json)
     
     if ($differences) {
         # Check if differences are only insignificant ones (like empty arrays vs missing fields)
         $significantDiff = $false
         foreach ($diff in $differences) {
             $line = $diff.InputObject.Trim()
             # Ignore differences that are just empty arrays or null values
             if ($line -notmatch '^\s*"[^"]+"\s*:\s*\[\s*\]\s*,?\s*$' -and $line -notmatch '^\s*"[^"]+"\s*:\s*null\s*,?\s*$') {
         ...
         if ($significantDiff) {
             Write-Host "`n⚠️ Warning - Resource was modified" -ForegroundColor Yellow
             $differences | Format-Table -AutoSize
         } else {
             Write-Host "`n✅ Verified - Resource unchanged (only insignificant differences like empty arrays)" -ForegroundColor Green
         }
     } else {
         Write-Host "`n✅ Verified - Resource unchanged" -ForegroundColor Green
     }
     ```
     
     **Bash**:
     ```bash
     # Filter insignificant fields recursively and compare
     diff <(jq --sort-keys 'walk(if type == "object" then del(.etag, .provisioningState, .resourceGuid, .timeCreated, .changedTime) else . end)' azurerm.json) \
          <(jq --sort-keys 'walk(if type == "object" then del(.etag, .provisioningState, .resourceGuid, .timeCreated, .changedTime) else . end)' azapi_step3.json)
     ```
     
     **Interpreting Results**:
     - ✅ **Verified - Resource unchanged**: Files are identical OR only have insignificant differences (empty arrays vs missing fields, timestamps, etags)
     - ⚠️ **Warning - Resource was modified**: Files show significant structural or value differences
...
2. **If plan shows changes (drift detected)**:
   ...
   **If ALL drifts are acceptable** (match patterns in the document):
   ...
   - If second plan shows no changes: 
     - **Verify actual resource hasn't been modified**: Retrieve resource status and compare to `azurerm.json`
