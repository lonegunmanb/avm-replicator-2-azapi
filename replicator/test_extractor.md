...
3. Ensure the repository root contains an `azurermacctest/` directory. If the folder is missing, create `azurermacctest` at the root before delegating any work. For every test case, locate a matching `azurermacctest/<case_name>` folder—if the exact name is absent, try camelCase ↔ snake_case conversions (for example, search `basicWithTags` and fall back to `basic_with_tags`, or vice versa). Create the resolved folder if it still does not exist and keep all extracted cases inside that path—do **not** create or use any other directory for this workflow.
4. Each Test Case Agent will:
  - **Part 1**: Ensure the resolved `azurermacctest/<case_name>` folder (including camelCase ↔ snake_case fallbacks) exists—create it if missing—and populate it with a complete `main.tf` file
  - **Part 2**: Convert AzureRM resources to AzAPI module, generating `azurerm.tf`, `azapi.tf.bak`, and `moved.tf.bak`
5. Once a test case agent finished both Part 1 and Part 2, mark the task as `Completed` and exit—do **not** run `terraform plan`, `terraform apply`, `terraform destroy`, or any other Terraform commands as part of the extraction workflow.
